# @kodo-x/analytics-browser

Kodo's Browser JavaScript SDK - send your frontend analytics data to the Kodo platform.
...
```bash
npm i @kodo-x/analytics-browser
```
...
- `events` - An array of objects. See the `track` method for details of the properties available for each object. Each event may optionally include an `idempotencyKey`; if omitted, one is generated automatically.
...
<head>
	...
	<script type="module" async>
		import Kodo from 'https://cdn.skypack.dev/@kodo-x/analytics-browser'
		Kodo.initialize('<YOUR_WRITE_KEY>', {
		    autoCapture: ['all'],
		    allowCookies: true,
		    autoEnrich: true,
